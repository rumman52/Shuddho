...
class LinkedInAgentProposalsActivationError(RuntimeError):
    pass
...
def load_json(path: Path, label: str) -> dict:
    try:
        ...
        raise LinkedInAgentProposalsActivationError(
...
def validate_staging(
    ...
    *,
    ...
    item = evidence.get("agent_linkedin_proposals")
    if not isinstance(item, dict):
        raise LinkedInAgentProposalsActivationError(
            "Staging evidence has no agent_linkedin_proposals result."
        )
    if item.get("status") != "passed":
        raise LinkedInAgentProposalsActivationError(
            "LinkedIn Agent-proposal staging evidence is not passed."
        )
    if (
        ...
        raise LinkedInAgentProposalsActivationError(
            "LinkedIn Agent-proposal staging evidence text is missing."
        )
    verified_at = item.get("verified_at")
    if not isinstance(verified_at, str):
        raise LinkedInAgentProposalsActivationError(
            "LinkedIn Agent-proposal staging evidence has no verified_at timestamp."
        )
    verified = parse_time(
        verified_at,
        "agent_linkedin_proposals verified_at",
    )
    age = (now - verified).total_seconds() / 60
    if age < -1:
        raise LinkedInAgentProposalsActivationError(
            "LinkedIn Agent-proposal staging evidence is from the future."
        )
    if age > max_age_minutes:
        raise LinkedInAgentProposalsActivationError(
            f"LinkedIn Agent-proposal staging evidence is stale ({age:.1f} minutes old)."
        )
    return verified
...
def validate_reviewed_rollout(
    ...
    *,
    ...
    if (
        ...
        raise LinkedInAgentProposalsActivationError(
            "Reviewed rollout manifest does not enable the full LinkedIn Agent-proposal dependency chain."
        )
    rollback = rollout["rollback"]
    if (
        rollback.get("agent_linkedin_proposals_kill_switch")
        != "SHUDDHO_AGENT_LINKEDIN_PROPOSALS_ENABLED=false"
    ):
        raise LinkedInAgentProposalsActivationError(
            "Reviewed rollout manifest has no exact LinkedIn Agent-proposals kill switch."
        )
...
def validate_deployment(
    ...
    *,
    ...
    if set(value) != DEPLOYMENT_KEYS:
        raise LinkedInAgentProposalsActivationError(
            "LinkedIn Agent-proposal deployment record has an unexpected schema."
        )
    for key in (
        "release_id",
        "change_reference",
    ...
    if value["staging_evidence_sha256"] != sha256_file(staging_path):
        raise LinkedInAgentProposalsActivationError(
            "Deployment record does not bind the exact LinkedIn Agent-proposal staging evidence."
        )
    ...
    deployed_at = parse_time(
        ...
        "LinkedIn Agent-proposal deployed_at",
    )
    if deployed_at < not_before:
        raise LinkedInAgentProposalsActivationError(
            "LinkedIn Agent-proposal deployment predates live staging evidence."
        )
    return deployed_at
...
def validate_operator_status(
    ...
    *,
    ...
    if generated < not_before:
        raise LinkedInAgentProposalsActivationError(
            "Operator status must be generated after LinkedIn Agent-proposal deployment."
        )
...
def validate_runtime_manifest(
    ...
    *,
    ...
    if (
        ...
        or capabilities.get("agent_runtime") is not True
        or capabilities.get("intelligent_planner") is not True
        or "linkedin" not in providers
    ):
        raise LinkedInAgentProposalsActivationError(
            "Deployed backend does not prove the full LinkedIn Agent-proposal dependency chain."
        )
    return {
...
def main() -> None:
    ...
    try:
        ...
        staging = load_json(
            ...
            "LinkedIn Agent-proposal staging evidence",
        )
        ...
        deployment = load_json(
            ...
            "LinkedIn Agent-proposal deployment",
        )

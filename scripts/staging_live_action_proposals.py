...
class ActionProposalValidationFailure(RuntimeError):
    pass
...
def require_guard() -> None:
    if (
        ...
        raise ActionProposalValidationFailure(
            "Set SHUDDHO_STAGING_ALLOW_LIVE_ACTION_PROPOSALS=true "
...
def connection_for(
    ...
    if (
        ...
        raise ActionProposalValidationFailure(
            f"{provider} email connection metadata is incomplete."
        )
    return value
...
def create_agent_run(
    ...
    goal = (
        "Create a short email draft and suggest exactly one inert email action "
        "proposal. Do not approve, send, execute, or select any provider/account. "
        f"The proposal recipient must be exactly {recipient}. "
        "Use no CC or BCC recipients. "
        f"The subject must include the exact marker {marker}. "
        f"The body must include the exact marker {marker}. "
        "Do not create a calendar proposal or any second action proposal. "
    ...
    return request_json(
        client.post(
            ...
            | {
                "Idempotency-Key":
                "live-action-proposals-" + uuid.uuid4().hex
            },
            json={
                "goal": goal,
                "output_language": "en",
        ...
        "create action-proposal Agent run",
        expected=202,
    )
...
def validate_inert_proposal(
    ...
    if run.get("planner_mode") != "intelligent":
        raise ActionProposalValidationFailure(
            "Live action-proposal probe did not use the intelligent planner."
        )
    if run.get("action_ids") != []:
        raise ActionProposalValidationFailure(
            "Action-proposal Agent run unexpectedly contains executable "
            "action bindings."
        )
    ...
    if not isinstance(proposals, list) or len(proposals) != 1:
        raise ActionProposalValidationFailure(
            "Expected exactly one live model-generated action proposal."
        )
    proposal = proposals[0]
    if not isinstance(proposal, dict):
        raise ActionProposalValidationFailure(
            "Action proposal has an unexpected shape."
        )
    if (
        proposal.get("kind") != "email_send"
        or proposal.get("state") != "suggested"
        or proposal.get("promoted_action_id") is not None
    ):
        raise ActionProposalValidationFailure(
            "New action proposal is not an inert suggested email proposal."
        )
    ...
    if not isinstance(payload, dict):
        raise ActionProposalValidationFailure(
            "Action proposal payload is missing."
        )
    if (
        payload.get("kind") != "email_send"
        or payload.get("to") != [recipient]
        or payload.get("cc") != []
        or payload.get("bcc") != []
        or not isinstance(payload.get("subject"), str)
        or marker not in payload["subject"]
        or not isinstance(payload.get("body"), str)
        or marker not in payload["body"]
    ):
        raise ActionProposalValidationFailure(
            "Model-generated email proposal does not match the explicitly "
            "requested staging scope."
        )
    if not isinstance(rationale, str) or not rationale.strip():
        raise ActionProposalValidationFailure(
            "Action proposal rationale is missing."
        )
    if (
        not isinstance(proposal_hash, str)
        or action_proposal_hash(
            str(run["id"]),
            payload,
            rationale,
        )
        ...
        raise ActionProposalValidationFailure(
            "Action proposal hash does not bind the exact run/payload/rationale."
        )
    ...
    for item in invocations:
        ...
        if (
            ...
            or item.get("tool") in {"email.send", "calendar.create"}
        ):
            raise ActionProposalValidationFailure(
                "Inert proposal generation inserted a consequential Agent step."
            )
    return proposal
...
def promote_exact(
    ...
    if (
        ...
        or preview.get("provider") != provider
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Validate live model-generated Agent action proposals remain inert "
    ...
    if args.provider == "microsoft" and not settings.microsoft_actions_enabled:
        raise SystemExit(
            "SHUDDHO_MICROSOFT_ACTIONS_ENABLED must be true when "
            "--provider microsoft is selected."
        )
    ...
    try:
        ...
        recipient = address(str(connection["email"]))
        ...
        proposal = validate_inert_proposal(
            planned,
            recipient,
            marker,
        )
        ...
        evidence = merge_evidence(
            args.base_evidence,
            {
                "action_proposals": passed(
                    "live intelligent Agent produced one inert typed email "
                    "proposal with no matching external action; wrong-hash "
                    "promotion was rejected; exact user-selected connection "
                    "promotion created exactly one standalone immutable "
                    "awaiting_approval preview; saved Agent plan remained "
                    "non-consequential; no approval, execution audit, or "
                    "provider receipt occurred"
                )
            },
        )
        ...
        print(json.dumps({
            ...
            "checks": {"action_proposals": "passed"},
        ...
        ActionProposalValidationFailure,
        httpx.HTTPError,
        OSError,
        ValueError,
    ) as error:

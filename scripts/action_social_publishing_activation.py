...
class ActionSocialPublishingActivationError(RuntimeError):
    pass
...
def validate_staging(
    ...
    *,
    ...
    item = evidence.get("action_social_publishing")
    if not isinstance(item, dict):
        raise ActionSocialPublishingActivationError(
            "Staging evidence has no action_social_publishing result."
        )
    if item.get("status") != "passed":
        raise ActionSocialPublishingActivationError(
            "Social-publishing staging evidence is not passed."
        )
    if not isinstance(item.get("evidence"), str) or not item["evidence"].strip():
        raise ActionSocialPublishingActivationError(
            "Social-publishing staging evidence text is missing."
        )
    verified_at = item.get("verified_at")
    if not isinstance(verified_at, str):
        raise ActionSocialPublishingActivationError(
            "Social-publishing staging evidence has no verified_at timestamp."
        )
    verified = parse_time(verified_at, "action_social_publishing verified_at")
    age = (now - verified).total_seconds() / 60
    if age < -1:
        raise ActionSocialPublishingActivationError(
            "Social-publishing staging evidence is from the future."
        )
    if age > max_age_minutes:
        raise ActionSocialPublishingActivationError(
            f"Social-publishing staging evidence is stale ({age:.1f} minutes old)."
        )
    return verified
...
def validate_reviewed_rollout(
    ...
    *,
    ...
    if (
        capabilities.get("action_social_publishing") is not True
        or capabilities.get("actions") is not True
    ):
        raise ActionSocialPublishingActivationError(
            "Reviewed rollout manifest does not enable the required actions/action_social_publishing capabilities."
        )
    rollback = rollout["rollback"]
    if (
        rollback.get("action_social_publishing_kill_switch")
        != "SHUDDHO_ACTION_SOCIAL_PUBLISHING_ENABLED=false"
    ):
        raise ActionSocialPublishingActivationError(
            "Reviewed rollout manifest has no exact social-publishing kill switch."
        )
    ...
    if "linkedin" not in providers:
        raise ActionSocialPublishingActivationError(
            "Reviewed rollout manifest does not include the LinkedIn action provider."
        )
    return {
...
def validate_deployment(
    ...
    *,
    ...
    if set(value) != DEPLOYMENT_KEYS:
        raise ActionSocialPublishingActivationError(
            "Social-publishing deployment record has an unexpected schema."
        )
    for key in (
        "release_id",
        "change_reference",
    ...
    if value["staging_evidence_sha256"] != sha256_file(staging_path):
        raise ActionSocialPublishingActivationError(
            "Deployment record does not bind the exact social-publishing staging evidence."
        )
    ...
    deployed_at = parse_time(
        ...
        "social-publishing deployed_at",
    )
    if deployed_at < not_before:
        raise ActionSocialPublishingActivationError(
            "Social-publishing deployment predates live staging evidence."
        )
    return deployed_at
...
def validate_operator_status(
    ...
    *,
    ...
    if generated < not_before:
        raise ActionSocialPublishingActivationError(
            "Operator status must be generated after social-publishing deployment."
        )
...
def validate_runtime_manifest(
    ...
    *,
    ...
    if capabilities.get("action_social_publishing") is not True:
        raise ActionSocialPublishingActivationError(
            "Deployed backend has social publishing disabled."
        )
    return {
...
def main() -> None:
    ...
    try:
        ...
        staging = load_json(
            ...
            "social-publishing staging evidence",
        )
        ...
        ActionSocialPublishingActivationError,
        OSError,
        ValueError,
    ) as error:
        print(json.dumps({

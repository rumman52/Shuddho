...
import { hasPendingMicrosoftCallback } from "./coworker/microsoftCallback";
import { hasPendingLinkedInCallback } from "./coworker/linkedinCallback";
...
export default function WorkspaceShell() {
  const [active, setActive] = useState<"writing" | "coworker">(() => {
    ...
    if (hasPendingGoogleCallback() || hasPendingMicrosoftCallback() || hasPendingLinkedInCallback() || window.location.hash.includes("type=recovery") || new URLSearchParams(window.location.search).has("code")) return "coworker";

...
export default function CoworkerWorkspace({ client, email, signOut }: { client: CoworkerClient; email: string; signOut: () => Promise<void> }) {
  const [view, setView] = useState<"drafts" | "goals" | "automations" | "actions" | "agent">(() => hasPendingGoogleCallback() || hasPendingMicrosoftCallback() || hasPendingLinkedInCallback() ? "actions" : "drafts");
  ...
  return <main className="cw-workspace">
    ...
    <nav className="cw-work-tabs" aria-label="Coworker services"><button aria-pressed={view === "drafts"} onClick={() => setView("drafts")}>Drafts & files</button><button aria-pressed={view === "goals"} onClick={() => setView("goals")}>Goals</button><button aria-pressed={view === "automations"} onClick={() => setView("automations")}>Automations</button><button aria-pressed={view === "agent"} onClick={() => setView("agent")}>Agent</button><button aria-pressed={view === "actions"} onClick={() => setView("actions")}>Email & calendar</button></nav>

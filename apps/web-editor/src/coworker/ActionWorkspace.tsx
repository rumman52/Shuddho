...
export default function ActionWorkspace({ client, account, emailDraft, focusActionId, onFocused }: { client: CoworkerClient; account: string; emailDraft: EmailDraft | null; focusActionId?: string | null; onFocused?: () => void }) {
  ...
  const [mode, setMode] = useState<"email" | "calendar" | "drive">("email");
  ...
  return <section className="cw-actions" aria-label={documentSharingEnabled ? "Email, calendar, and document sharing actions" : "Email and calendar actions"}>
    ...
    <div className="cw-connection-grid">{(provider === "google" ? (["email", "calendar", "drive"] as const) : (["email", "calendar"] as const)).map(capability => {
      ...
      const label = capability === "email"
        ? (google ? "Gmail" : "Outlook / Microsoft Mail")
        : capability === "calendar"
          ? (google ? "Google Calendar" : "Microsoft Calendar")
          : "Google Drive";
      const controlLabel = microsoftEnabled ? label : capability === "email" ? "Gmail" : capability === "calendar" ? "Calendar" : "Google Drive";
      return <div className="cw-connection" key={provider + capability}><div><strong>{label}</strong><small>{connected?.email ?? (capability === "email" ? "Send emails you approve" : capability === "calendar" ? "Create events in your primary calendar" : "Share one approved Shuddho artifact")}</small></div>
        ...
        }}>Disconnect {controlLabel}</button> : <button className="cw-secondary" disabled={!enabled || Boolean(busy)} onClick={() => void run("connect", () => provider === "google" ? beginGoogleConnection(client, account, capability) : capability === "drive" ? Promise.reject(new WorkspaceError("Google Drive sharing requires Google.")) : beginMicrosoftConnection(client, account, capability))}>Connect {controlLabel}</button>}
      </div>;
    })}</div>
    ...
    <div className="cw-layout"><div className="cw-compose cw-action-compose">
      ...
      {composing ? <form onSubmit={prepare}>
        ...
        <div className="cw-action-content" dir="auto"><h3>{title(action)}</h3>{payload?.kind !== "document_share" && <p>{payload && "title" in payload ? payload.description : payload?.body}</p>}</div>
    ...
    <div className="cw-output-column"><section className="cw-result cw-action-review" aria-label="Action review">
      ...
      {composing || !action ? <p>Your preview will appear here. Check the full message or event details before approving.</p> : <>
        ...
        {action.state === "awaiting_approval" && <>
          ...
          <label className="cw-approval-check"><input type="checkbox" checked={checked} onChange={event => setChecked(event.target.checked)} />{isDocumentShare(action.kind) ? "I reviewed the account, exact document hash, recipient and reader access." : "I reviewed the account, recipients, content and timing."}</label>
          <button className="cw-primary" disabled={!checked || !enabled || Boolean(busy)} onClick={() => void run("approve", async () => updateAction(await client.approveAction(action)))}>{busy === "approve" ? "Approving…" : isEmail(action.kind) ? "Approve & send email" : isDocumentShare(action.kind) ? "Approve & share document" : "Approve & create event"}</button>
        </>}

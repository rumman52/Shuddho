...
export default function ActionWorkspace({ client, account, emailDraft }: { client: CoworkerClient; account: string; emailDraft: EmailDraft | null }) {
  ...
  function remember(value: ExternalAction) {
  ...
  async function prepare(event: FormEvent) {
    ...
    await run("prepare", async () => remember(await client.prepareAction(input, submission.current!.key)));
  }
  ...
  async function edit() {
    ...
    await run("edit", async () => {
      ...
      setAction(null); setChecked(false); submission.current = undefined; setReload(x => x + 1);
    });
  }
  ...
  return <section className="cw-actions" aria-label="Email and calendar actions">
    ...
    <div className="cw-connection-grid">{(["email", "calendar"] as const).map(capability => {
      ...
      return <div className="cw-connection" key={capability}><div><strong>{capability === "email" ? "Gmail" : "Google Calendar"}</strong><small>{connected?.email ?? (capability === "email" ? "Send emails you approve" : "Create events in your primary calendar")}</small></div>
        {connected ? <button className="cw-text-button" disabled={Boolean(busy)} onClick={() => {
          ...
          void run("disconnect", async () => { const result = await client.disconnect(connected.id); setNotice(result.message); setReload(x => x + 1); if (action) remember(await client.action(action.id)); });
    ...
    <div className="cw-layout"><div className="cw-compose cw-action-compose">
      <div className="cw-card-title"><span className="cw-step-number">01</span><div><h2>{action ? "Action details" : "Prepare an action"}</h2><p>{action ? "This preview is saved exactly as shown." : "Nothing is sent when you prepare a preview."}</p></div></div>
      {!action ? <form onSubmit={prepare}>
        ...
        {action.state === "awaiting_approval" && <button className="cw-secondary" disabled={Boolean(busy)} onClick={() => void edit()}>Edit details</button>}
      </>}
    </div>
    <div className="cw-output-column"><section className="cw-result cw-action-review" aria-label="Action review">
      ...
      {!action ? <p>Your preview will appear here. Check the full message or event details before approving.</p> : <>
        ...
        {action.state === "awaiting_approval" && <>
          ...
          <button className="cw-primary" disabled={!checked || !enabled || Boolean(busy)} onClick={() => void run("approve", async () => remember(await client.approveAction(action)))}>{busy === "approve" ? "Approving…" : action.kind === "email_send" ? "Approve & send email" : "Approve & create event"}</button>
        </>}
        {["awaiting_approval", "queued"].includes(action.state) && <button className="cw-text-button cw-cancel" disabled={Boolean(busy)} onClick={() => void run("cancel", async () => remember(await client.cancelAction(action.id)))}>Cancel action</button>}
        {action.state === "outcome_unknown" && action.kind === "calendar_create" && <button className="cw-secondary" disabled={Boolean(busy) || !enabled} onClick={() => void run("reconcile", async () => remember(await client.reconcileAction(action.id)))}>{busy === "reconcile" ? "Checking calendar…" : "Check calendar result"}</button>}
    ...
    <section className="cw-history"><div className="cw-history-title"><h2>Recent actions</h2><button className="cw-text-button" onClick={() => { setReload(x => x + 1); if (action) void run("refresh", async () => remember(await client.action(action.id))); }}>Refresh actions</button></div>
      {history.length ? <ul>{history.map(item => <li key={item.id}><button aria-pressed={action?.id === item.id} disabled={Boolean(busy)} onClick={() => void run("open", async () => remember(await client.action(item.id)))}><div><strong dir="auto">{title(item)}</strong><small>{item.kind === "email_send" ? "Email" : "Calendar"} · {item.preview.account}</small></div><span className="cw-status">{labels[item.state]}</span></button></li>)}</ul> : <p className="cw-fineprint">Your approved actions and receipts will appear here.</p>}
      {action && <button className="cw-secondary cw-new-action" disabled={Boolean(busy)} onClick={() => { setAction(null); setChecked(false); submission.current = undefined; }}>Prepare another action</button>}
    </section></div></div>
  </section>;
}

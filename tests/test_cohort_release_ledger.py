...
def action_document_sharing_files(tmp_path, *, current_stage="cohort-25"):
    capabilities = {
        "coworker": True,
        "work_services": True,
        "artifact_services": True,
        ...
        "action_document_sharing": True,
        "action_selection": False,
        "action_proposals": False,
    }
    rollout = write_json(tmp_path / ("document-sharing-" + current_stage + "-rollout.json"), {
        "release_id": "coworker-cohort-001",
        "environment": "production",
        "cohort": {"reference": "approved-cohort", "max_users": 25},
        "capabilities": capabilities,
        ...
        "incident": {"change_reference": "document-sharing-change-1"},
    })
    staging = write_json(tmp_path / ("document-sharing-" + current_stage + "-staging.json"), {
        "action_document_sharing": {
            "status": "passed",
            "evidence": "exact Drive artifact and reader permission passed",
            "verified_at": "2026-09-24T08:00:00+00:00",
        },
    })
    deployment = write_json(tmp_path / ("document-sharing-" + current_stage + "-deployment.json"), {
        "release_id": "coworker-cohort-001",
        "change_reference": "document-sharing-change-1",
        "current_stage": current_stage,
        "deployed_at": "2026-09-24T08:10:00+00:00",
        "source_revision": "e" * 40,
        "staging_evidence_sha256": file_sha256(staging),
        "rollout_manifest_sha256": file_sha256(rollout),
    })
    status = write_json(tmp_path / ("document-sharing-" + current_stage + "-status.json"), {
        "release_id": "coworker-cohort-001",
        "decision": "CONTINUE_COHORT",
        "generated_at": "2026-09-24T08:15:00+00:00",
        "breaches": [],
    })
    runtime = {
        "schema_version": 1,
    ...
    activation = write_json(tmp_path / ("document-sharing-" + current_stage + "-activation.json"), {
        "schema_version": 1,
        ...
        "deployed_at": "2026-09-24T08:10:00+00:00",
        "source_revision": "e" * 40,
        "operator_status_generated_at": "2026-09-24T08:15:00+00:00",
        "runtime": runtime,
        "runtime_manifest_sha256": runtime_hash,
        "artifact_sha256": {
            "staging_evidence": file_sha256(staging),
...
def test_schema_v12_document_sharing_is_hash_chained(tmp_path):
    ledger = tmp_path / "release-ledger-document-sharing.jsonl"
    seed_cohort_25_ledger(ledger, tmp_path)
    staging, rollout, deployment, status, activation = action_document_sharing_files(tmp_path)
    ...
    entry = append_action_document_sharing_event(
        ...
        deployment_change=deployment,
        operator_status=status,
        action_document_sharing_activation=activation,
        created_at="2026-09-24T08:25:00+00:00",
    )

    assert entry["schema_version"] == 12
    assert entry["event_type"] == "action_document_sharing_verified"
    assert entry["artifact_sha256"]["action_document_sharing_activation"] == file_sha256(activation)
    assert verify_entries(read_entries(ledger), KEY)["head_entry_hash"] == entry["entry_hash"]


def test_schema_v12_document_sharing_requires_exact_kill_switch(tmp_path):
    ledger = tmp_path / "release-ledger-document-sharing-broken.jsonl"
    seed_cohort_25_ledger(ledger, tmp_path)
    staging, rollout, deployment, status, activation = action_document_sharing_files(tmp_path)
    value = json.loads(rollout.read_text(encoding="utf-8"))
    value["rollback"]["action_document_sharing_kill_switch"] = "wrong"
    rollout.write_text(json.dumps(value), encoding="utf-8")
    deployment_value = json.loads(deployment.read_text(encoding="utf-8"))
    deployment_value["rollout_manifest_sha256"] = file_sha256(rollout)
    deployment.write_text(json.dumps(deployment_value), encoding="utf-8")
...
def action_email_threading_files(tmp_path, *, current_stage="cohort-25"):
    capabilities = {
        "coworker": True,
        "work_services": True,
        "artifact_services": True,
        ...
        "action_email_threading": True,
        "action_selection": False,
        "action_proposals": False,
    }
    rollout = write_json(tmp_path / ("email-threading-" + current_stage + "-rollout.json"), {
        "release_id": "coworker-cohort-001",
        "environment": "production",
        "cohort": {"reference": "approved-cohort", "max_users": 25},
        "capabilities": capabilities,
        ...
        "incident": {"change_reference": "email-threading-change-1"},
    })
    staging = write_json(tmp_path / ("email-threading-" + current_stage + "-staging.json"), {
        "action_email_threading": {
            "status": "passed",
            "evidence": "exact Gmail owned thread and stable thread id passed",
            "verified_at": "2026-09-24T08:00:00+00:00",
        },
    })
    deployment = write_json(tmp_path / ("email-threading-" + current_stage + "-deployment.json"), {
        "release_id": "coworker-cohort-001",
        "change_reference": "email-threading-change-1",
        "current_stage": current_stage,
        "deployed_at": "2026-09-24T08:10:00+00:00",
        "source_revision": "e" * 40,
        "staging_evidence_sha256": file_sha256(staging),
    ...
    status = write_json(tmp_path / ("email-threading-" + current_stage + "-status.json"), {
        "release_id": "coworker-cohort-001",
        "decision": "CONTINUE_COHORT",
        "generated_at": "2026-09-24T08:15:00+00:00",
        "breaches": [],
    ...
    activation = write_json(tmp_path / ("email-threading-" + current_stage + "-activation.json"), {
        "schema_version": 1,
...
def test_schema_v13_document_sharing_is_hash_chained(tmp_path):
    ledger = tmp_path / "release-ledger-email-threading.jsonl"
    seed_cohort_25_ledger(ledger, tmp_path)
    staging, rollout, deployment, status, activation = action_email_threading_files(tmp_path)
    ...
    entry = append_action_email_threading_event(
        ...
        deployment_change=deployment,
        operator_status=status,
        action_email_threading_activation=activation,
        created_at="2026-09-24T08:25:00+00:00",
    )

    assert entry["schema_version"] == 13
    assert entry["event_type"] == "action_email_threading_verified"
    assert entry["artifact_sha256"]["action_email_threading_activation"] == file_sha256(activation)
    assert verify_entries(read_entries(ledger), KEY)["head_entry_hash"] == entry["entry_hash"]


def test_schema_v13_document_sharing_requires_exact_kill_switch(tmp_path):
    ledger = tmp_path / "release-ledger-email-threading-broken.jsonl"
    seed_cohort_25_ledger(ledger, tmp_path)
    staging, rollout, deployment, status, activation = action_email_threading_files(tmp_path)
    value = json.loads(rollout.read_text(encoding="utf-8"))
    value["rollback"]["action_email_threading_kill_switch"] = "wrong"
    rollout.write_text(json.dumps(value), encoding="utf-8")
    deployment_value = json.loads(deployment.read_text(encoding="utf-8"))
    deployment_value["rollout_manifest_sha256"] = file_sha256(rollout)
    deployment.write_text(json.dumps(deployment_value), encoding="utf-8")
...
def action_social_publishing_files(tmp_path, *, current_stage="cohort-25"):
    capabilities = {
        "coworker": True,
        "work_services": True,
        "artifact_services": True,
    ...
    rollout = write_json(tmp_path / ("social-" + current_stage + "-rollout.json"), {
        "release_id": "coworker-cohort-001",
        "environment": "production",
        "cohort": {"reference": "approved-cohort", "max_users": 25},
        "capabilities": capabilities,
        ...
        "incident": {"change_reference": "social-publishing-change-1"},
    })
    staging = write_json(tmp_path / ("social-" + current_stage + "-staging.json"), {
        "action_social_publishing": {
            "status": "passed",
            "evidence": "exact LinkedIn personal text post passed",
            "verified_at": "2026-09-24T09:00:00+00:00",
        },
    })
    deployment = write_json(tmp_path / ("social-" + current_stage + "-deployment.json"), {
        "release_id": "coworker-cohort-001",
        "change_reference": "social-publishing-change-1",
        "current_stage": current_stage,
        "deployed_at": "2026-09-24T09:10:00+00:00",
        "source_revision": "f" * 40,
        "staging_evidence_sha256": file_sha256(staging),
        "rollout_manifest_sha256": file_sha256(rollout),
    })
    status = write_json(tmp_path / ("social-" + current_stage + "-status.json"), {
        "release_id": "coworker-cohort-001",
        "decision": "CONTINUE_COHORT",
        "generated_at": "2026-09-24T09:15:00+00:00",
        "breaches": [],
    })
    runtime = {
        "schema_version": 1,
    ...
    activation = write_json(tmp_path / ("social-" + current_stage + "-activation.json"), {
        "schema_version": 1,
        "status": "action_social_publishing_verified",
        "release_id": "coworker-cohort-001",
        "current_stage": current_stage,
        "verified_at": "2026-09-24T09:20:00+00:00",
        "change_reference": "social-publishing-change-1",
        "deployed_at": "2026-09-24T09:10:00+00:00",
        "source_revision": "f" * 40,
        "operator_status_generated_at": "2026-09-24T09:15:00+00:00",
        "runtime": runtime,
        "runtime_manifest_sha256": runtime_hash,
        "artifact_sha256": {
            "staging_evidence": file_sha256(staging),
...
def test_schema_v14_social_publishing_is_hash_chained_and_unique(tmp_path):
    ledger = tmp_path / "release-ledger-social.jsonl"
    seed_cohort_25_ledger(ledger, tmp_path)
    staging, rollout, deployment, status, activation = action_social_publishing_files(tmp_path)
    ...
    assert entry["schema_version"] == 14
    assert entry["event_type"] == "action_social_publishing_verified"
    assert entry["artifact_sha256"]["action_social_publishing_activation"] == file_sha256(activation)
    assert verify_entries(read_entries(ledger), KEY)["head_entry_hash"] == entry["entry_hash"]
...
def test_schema_v14_social_publishing_requires_exact_kill_switch(tmp_path):
    ledger = tmp_path / "release-ledger-social-broken.jsonl"
    seed_cohort_25_ledger(ledger, tmp_path)
    staging, rollout, deployment, status, activation = action_social_publishing_files(tmp_path)
    value = json.loads(rollout.read_text(encoding="utf-8"))
    value["rollback"]["action_social_publishing_kill_switch"] = "wrong"
    rollout.write_text(json.dumps(value), encoding="utf-8")
    deployment_value = json.loads(deployment.read_text(encoding="utf-8"))
    deployment_value["rollout_manifest_sha256"] = file_sha256(rollout)
    deployment.write_text(json.dumps(deployment_value), encoding="utf-8")

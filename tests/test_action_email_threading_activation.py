...
from scripts.action_email_threading_activation import (
...
def test_reviewed_rollout_requires_exact_email_threading_kill_switch():
    reviewed = validate_reviewed_rollout(rollout(), max_cohort_users=25)
    assert reviewed["capabilities"]["action_email_threading"] is True
    broken = rollout()
    broken["rollback"]["action_email_threading_kill_switch"] = "wrong"
    with pytest.raises(ActionEmailThreadingActivationError):
...
def test_staging_is_timestamped_fresh_and_passed():
    ...
    stale = staging()
    stale["action_email_threading"]["verified_at"] = (
        NOW - timedelta(minutes=61)
    ).isoformat()
    with pytest.raises(ActionEmailThreadingActivationError, match="stale"):
        validate_staging(stale, now=NOW, max_age_minutes=60)


def test_runtime_manifest_exactly_matches_reviewed_email_threading_rollout():
    reviewed = validate_reviewed_rollout(rollout(), max_cohort_users=25)
    ...
    assert result["capabilities"]["action_email_threading"] is True

    drifted = json.loads(json.dumps(value))
    drifted["capabilities"]["action_email_threading"] = False
    with pytest.raises(ActionEmailThreadingActivationError, match="capability flags"):
        validate_runtime_manifest(
            drifted,
            deployment={"source_revision": REVISION},
            rollout=reviewed,
...
def test_activation_evidence_binds_inputs(tmp_path):
    ...
    assert evidence["status"] == "action_email_threading_verified"

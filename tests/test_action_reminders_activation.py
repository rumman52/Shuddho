...
from scripts.action_reminders_activation import (
...
def rollout(providers=None):
    ...
    return {
        ...
        "incident": {"oncall_reference": "oncall", "change_reference": "reminder-change-1"},
    }


def staging(providers=None):
    providers = providers or ["google"]
    return {
        f"action_reminders_{provider}": {
            "status": "passed",
            "evidence": f"live {provider} reminder proof",
...
def test_reviewed_rollout_requires_exact_reminder_kill_switch():
    reviewed = validate_reviewed_rollout(rollout(), max_cohort_users=25)
    assert reviewed["capabilities"]["action_reminders"] is True
    broken = rollout()
    broken["rollback"]["action_reminders_kill_switch"] = "wrong"
    with pytest.raises(ActionRemindersActivationError):
        validate_reviewed_rollout(broken, max_cohort_users=25)


def test_staging_requires_every_declared_provider():
    ...
    with pytest.raises(ActionRemindersActivationError, match="microsoft"):
        validate_staging(
            staging(["google"]),
...
def test_runtime_manifest_exactly_matches_reviewed_reminder_rollout():
    reviewed = validate_reviewed_rollout(rollout(), max_cohort_users=25)
    ...
    assert result["capabilities"]["action_reminders"] is True

    drifted = json.loads(json.dumps(value))
    drifted["capabilities"]["action_reminders"] = False
    with pytest.raises(ActionRemindersActivationError, match="capability flags"):
        validate_runtime_manifest(
            drifted,
            deployment={"source_revision": REVISION},
            rollout=reviewed,
...
def test_activation_evidence_binds_inputs(tmp_path):
    ...
    deployment = {
        "release_id": "coworker-cohort-001",
        "change_reference": "reminder-change-1",
    ...
    assert evidence["status"] == "action_reminders_verified"

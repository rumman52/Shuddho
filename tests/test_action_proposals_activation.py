...
from scripts.action_proposals_activation import (
...
def rollout():
    return {
        ...
        "incident": {
            ...
            "change_reference": "change-action-proposals-001",
        },
    }
...
def test_reviewed_rollout_requires_action_proposals_and_exact_kill_switch():
    value = rollout()
    ...
    with pytest.raises(
        ActionProposalsActivationError,
        match="does not enable action_proposals",
    ):
        validate_reviewed_rollout(
            value,
            max_cohort_users=25,
    ...
    value = rollout()
    value["rollback"]["action_proposals_kill_switch"] = "wrong"
    with pytest.raises(
        ActionProposalsActivationError,
        match="invalid",
    ):
...
def test_runtime_manifest_must_exactly_match_reviewed_deployment():
    ...
    assert result["capabilities"]["action_proposals"] is True
...
def test_activation_evidence_binds_all_inputs_and_remote_runtime(tmp_path):
    ...
    assert evidence["status"] == "action_proposals_verified"

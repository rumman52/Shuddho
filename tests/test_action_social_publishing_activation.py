...
from scripts.action_social_publishing_activation import (
...
def staging():
    return {
        "action_social_publishing": {
            "status": "passed",
            "evidence": "live approval-bound LinkedIn text post passed",
...
def test_reviewed_rollout_requires_exact_social_kill_switch_and_linkedin_provider():
    reviewed = validate_reviewed_rollout(rollout(), max_cohort_users=25)
    assert reviewed["capabilities"]["action_social_publishing"] is True
    assert reviewed["action_providers"] == ["google", "linkedin"]

    broken = rollout()
    broken["rollback"]["action_social_publishing_kill_switch"] = "wrong"
    with pytest.raises(ActionSocialPublishingActivationError):
        validate_reviewed_rollout(broken, max_cohort_users=25)

    missing = rollout()
    missing["action_providers"] = ["google"]
    with pytest.raises(ActionSocialPublishingActivationError):
        validate_reviewed_rollout(missing, max_cohort_users=25)
...
def test_staging_is_timestamped_fresh_and_passed():
    ...
    stale = staging()
    stale["action_social_publishing"]["verified_at"] = (
        NOW - timedelta(minutes=61)
    ).isoformat()
    with pytest.raises(ActionSocialPublishingActivationError, match="stale"):
        validate_staging(stale, now=NOW, max_age_minutes=60)


def test_runtime_manifest_exactly_matches_reviewed_social_rollout():
    reviewed = validate_reviewed_rollout(rollout(), max_cohort_users=25)
    ...
    assert result["capabilities"]["action_social_publishing"] is True
    assert "linkedin" in result["action_providers"]

    drifted = json.loads(json.dumps(value))
    drifted["capabilities"]["action_social_publishing"] = False
    with pytest.raises(ActionSocialPublishingActivationError, match="capability flags"):
        validate_runtime_manifest(
            drifted,
            deployment={"source_revision": REVISION},
            rollout=reviewed,
...
def test_activation_evidence_binds_exact_inputs(tmp_path):
    ...
    assert evidence["status"] == "action_social_publishing_verified"

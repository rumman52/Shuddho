...
from scripts.action_document_sharing_activation import (
...
def staging():
    return {
        "action_document_sharing": {
            "status": "passed",
            "evidence": "live exact artifact hash and reader permission passed",
...
def test_reviewed_rollout_requires_exact_document_sharing_kill_switch():
    reviewed = validate_reviewed_rollout(rollout(), max_cohort_users=25)
    assert reviewed["capabilities"]["action_document_sharing"] is True
    broken = rollout()
    broken["rollback"]["action_document_sharing_kill_switch"] = "wrong"
    with pytest.raises(ActionDocumentSharingActivationError):
        validate_reviewed_rollout(broken, max_cohort_users=25)


def test_reviewed_rollout_requires_artifact_services():
    broken = rollout()
    broken["capabilities"]["artifact_services"] = False
    with pytest.raises(ActionDocumentSharingActivationError):
        validate_reviewed_rollout(broken, max_cohort_users=25)
...
def test_staging_is_timestamped_fresh_and_passed():
    ...
    stale = staging()
    stale["action_document_sharing"]["verified_at"] = (
        NOW - timedelta(minutes=61)
    ).isoformat()
    with pytest.raises(ActionDocumentSharingActivationError, match="stale"):
        validate_staging(stale, now=NOW, max_age_minutes=60)


def test_runtime_manifest_exactly_matches_reviewed_document_sharing_rollout():
    reviewed = validate_reviewed_rollout(rollout(), max_cohort_users=25)
    ...
    assert result["capabilities"]["action_document_sharing"] is True

    drifted = json.loads(json.dumps(value))
    drifted["capabilities"]["action_document_sharing"] = False
    with pytest.raises(ActionDocumentSharingActivationError, match="capability flags"):
        validate_runtime_manifest(
            drifted,
            deployment={"source_revision": REVISION},
            rollout=reviewed,
...
def test_activation_evidence_binds_inputs(tmp_path):
    ...
    assert evidence["status"] == "action_document_sharing_verified"

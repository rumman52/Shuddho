...
PA-01 and PA-02 are merged on `main` through PRs #207 and #208; their exact implementation heads passed repository CI and remain default-off pending separate staging/production qualification. PA-03 is implemented on `codex/personal-agent-pa03-runtime-v3` pending review and CI: new eligible runs freeze their runtime version at admission, `shuddho_agent_run_v3` consumes only server-verified observations, DeepSeek returns one typed bounded decision at a time, decision/model/prompt/tool-schema evidence is persisted, planner call/token/cost accounting is explicit, exact receipts govern completion, and rollback of the v3 flag affects new admission without replaying existing v3 histories through v1/v2. The existing v1/v2 workflows remain registered unchanged. PA-04 is the next architecture increment only after PA-03 is reviewed, CI-verified and merged. PA-03/04 improve reasoning and context without granting browser or payment authority. PA-05 is a prerequisite for general browser/code access and broader credential use. PA-09 requires the relevant qualified API or browser path; a browser feature alone is insufficient.
...
**Proposed capability flags must be implemented through the canonical release contract.** The following names are design proposals and do not exist merely because they appear here.
...
| `SHUDDHO_CONNECTOR_READS_ENABLED` | Qualified connected read operations |
...
Implementation adds each flag's dependencies, providers, live evidence, activation, ledger, rollback and recovery rules to `scripts/release_contract.py`. A broad transaction flag cannot replace per-provider/per-operation allowlisting. No new flag defaults on. Preserve current coworker and LinkedIn Agent proposal production restrictions.

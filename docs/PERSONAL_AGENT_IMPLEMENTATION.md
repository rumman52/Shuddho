...
PA-01 is implemented on the dedicated `codex/personal-agent-pa01-persistent-goals` branch as a default-off capability pending review, CI, controlled staging and merge. It adds owner-scoped goals, immutable revisions, bounded-run revision binding, lifecycle controls, a Goals UI and canonical release-contract gating without adding provider authority. PA-02 remains the next architecture increment after PA-01 is reviewed and merged. PA-02 adds the first new visible automation using existing non-consequential services. PA-03/04 improve reasoning and context without granting browser or payment authority. PA-05 is a prerequisite for general browser/code access and broader credential use. PA-09 requires the relevant qualified API or browser path; a browser feature alone is insufficient.

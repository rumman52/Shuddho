...
PA-01 is the next code PR. PA-02 adds the first new visible automation using existing non-consequential services. PA-03/04 improve reasoning and context without granting browser or payment authority. PA-05 is a prerequisite for general browser/code access and broader credential use. PA-09 requires the relevant qualified API or browser path; a browser feature alone is insufficient.

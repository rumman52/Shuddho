...
PA-01 is merged on `main` through PR #207 and its exact-head CI rerun passed. It remains default-off and not staging-verified or production-enabled. PA-02 is implemented on `codex/personal-agent-pa02-automations-notifications` pending review and CI: owner-scoped automation revisions are stored in PostgreSQL, a reconciliation outbox maps desired state to Temporal Schedules, canonical occurrences dedupe to bounded goal-backed AgentRuns, application-level overlap/catch-up/expiry rules are enforced, and durable in-app notifications honor quiet hours. Temporal remains the only due-time firing authority; notification visibility and schedule reconciliation use outboxes but do not create a second timer that can fire work. PA-03 remains next only after PA-02 is merged and verified. PA-03/04 improve reasoning and context without granting browser or payment authority. PA-05 is a prerequisite for general browser/code access and broader credential use. PA-09 requires the relevant qualified API or browser path; a browser feature alone is insufficient.

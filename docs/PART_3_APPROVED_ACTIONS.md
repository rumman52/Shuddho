...
[Approved Gmail Owned-Thread Follow-ups](APPROVED_EMAIL_THREADING.md) is the next bounded Wave D implementation. It is disabled by default behind `SHUDDHO_ACTION_EMAIL_THREADING_ENABLED=false`. A user may continue only a confirmed Gmail thread created by an earlier Shuddho send on the same Google email connection. The parent action, Gmail thread id, deterministic RFC Message-ID chain, exact To/Cc recipients and unchanged subject are copied into the immutable preview and approval scope. Bcc, attachments, arbitrary thread ids, mailbox reads, Microsoft threading and Agent-selected threading remain out of scope. This implementation is **not production-release-qualified yet**; a dedicated live Gmail threading/activation/release-ledger evidence chain must be added before the flag can enter a production cohort.

Pending Wave D work after this implementation: threading release qualification and social publishing. Social publishing must register through the same consequential-action policy and pass the same approval/receipt boundary rather than acquiring permission from model-generated text.

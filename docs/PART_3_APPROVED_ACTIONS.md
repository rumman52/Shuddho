...
| --- | --- | --- |
| Email | Gmail/Google Workspace account, To/Cc/Bcc, multilingual subject and plain-text body, immediate sending after approval; optional already-owned Shuddho artifact attachments behind the separate attachment flag; Google-only follow-ups inside a confirmed Shuddho-sent Gmail thread behind the separate threading flag | Up to 20 distinct recipients; attachments are capped at 3 / 2 MiB and must be immutable owned outputs; threaded follow-ups preserve the exact parent To/Cc and subject, forbid Bcc/attachments, and add no mailbox-read scope; no sender aliases or scheduled sends |
...
| `GET /api/v1/artifacts` | Owner-scoped generated artifact catalog for explicit attachment selection |
...
The next bounded Wave D action increment adds [Approved Calendar Reminders](APPROVED_CALENDAR_REMINDERS.md), disabled by default behind `SHUDDHO_ACTION_REMINDERS_ENABLED=false`. A user may choose one bounded reminder on a future calendar event; exact minutes are approval-hash-bound and independently checked in Google/Microsoft provider receipts. Agent proposals remain reminder-blind. Production enablement now requires provider-specific live reminder evidence, exact deployment/runtime activation, schema-v10 `action_reminders_verified` release-ledger attestation, and schema-v5 scale/recovery consumption.

The next bounded Wave D usability increment adds [Owned Action Recipient Directory](APPROVED_ACTION_RECIPIENTS.md). It stores a bounded set of user-created name/email shortcuts inside Shuddho, adds no Google/Microsoft Contacts or mailbox-read scope, and never resolves names inside the executor. Selecting a saved recipient copies the exact validated email into the ordinary action draft, so the existing immutable preview, approval hash and provider receipt boundary remain unchanged. The directory is disabled by default behind `SHUDDHO_ACTION_RECIPIENTS_ENABLED=false`; Agents cannot read or select it.

[Approved Google Drive Document Sharing](APPROVED_DOCUMENT_SHARING.md) is implemented and release-qualified for a reviewed controlled cohort, while remaining disabled by default behind `SHUDDHO_ACTION_DOCUMENT_SHARING_ENABLED=false`. It shares one immutable owned Shuddho artifact with one exact email as a reader using only Google `drive.file`; no broad Drive read, folder browsing, public link, writer role, contact lookup, or Agent access is introduced. Production enablement requires the dedicated live staging, activation, schema-v12 ledger, and schema-v7 scale/recovery evidence chain.

[Approved Gmail Owned-Thread Follow-ups](APPROVED_EMAIL_THREADING.md) is implemented and release-qualified for a reviewed controlled cohort, while remaining disabled by default behind `SHUDDHO_ACTION_EMAIL_THREADING_ENABLED=false`. A user may continue only a confirmed Gmail thread created by an earlier Shuddho send on the same Google email connection. The parent action, Gmail thread id, deterministic RFC Message-ID chain, exact To/Cc recipients and unchanged subject are approval-bound. Bcc, attachments, arbitrary thread ids, mailbox reads, Microsoft threading and Agent-selected threading remain out of scope. Production enablement requires guarded live Gmail evidence, exact runtime activation, schema-v13 `action_email_threading_verified` release-ledger attestation, and schema-v8 scale/recovery consumption.

[Approved LinkedIn Social Publishing v1](APPROVED_SOCIAL_PUBLISHING.md) is now implemented behind `SHUDDHO_ACTION_SOCIAL_PUBLISHING_ENABLED=false`. It publishes only one explicitly approved public text post to the connected personal LinkedIn member. Exact author identity, post text, public visibility and immediate timing are approval-bound. Media, organization pages, social reads, scheduling, edits/deletes, engagement actions and Agent authority remain out of scope. It is release-qualified for a reviewed controlled cohort while remaining disabled by default. Production enablement requires guarded live LinkedIn staging, exact runtime activation, schema-v14 `action_social_publishing_verified` release-ledger attestation, and schema-v9 scale/recovery consumption.

[Inert LinkedIn Agent Action Proposals](AGENT_LINKEDIN_PROPOSALS.md) extends the already non-executable proposal object to exact personal LinkedIn text only, behind the nested `SHUDDHO_AGENT_LINKEDIN_PROPOSALS_ENABLED=false` gate. The model still cannot select a connection/member, approve, or publish; the user must promote into the existing immutable `social_publish_linkedin` preview and approve separately. It is release-qualified for a reviewed controlled cohort while remaining disabled by default. Production enablement requires dedicated live inert-proposal staging, exact runtime activation, schema-v15 `agent_linkedin_proposals_verified` release-ledger attestation, and schema-v10 scale/recovery consumption. Organization publishing, media, social reads, scheduling, edits/deletes, engagement actions, and direct Agent publishing remain separate future decisions.

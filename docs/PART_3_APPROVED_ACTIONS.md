...
[Approved LinkedIn Social Publishing v1](APPROVED_SOCIAL_PUBLISHING.md) is now implemented behind `SHUDDHO_ACTION_SOCIAL_PUBLISHING_ENABLED=false`. It publishes only one explicitly approved public text post to the connected personal LinkedIn member. Exact author identity, post text, public visibility and immediate timing are approval-bound. Media, organization pages, social reads, scheduling, edits/deletes, engagement actions and Agent authority remain out of scope. This implementation is **not production-release-qualified yet**.

Pending Wave D work after this implementation: LinkedIn social publishing release qualification with guarded live staging, exact activation proof, kill-switch verification, release-ledger attestation, and scale/recovery consumption.

...
The next bounded architecture increment is [Inert LinkedIn Agent Action Proposals](AGENT_LINKEDIN_PROPOSALS.md). It extends the already non-executable proposal object to exact personal LinkedIn text only, behind a new nested `SHUDDHO_AGENT_LINKEDIN_PROPOSALS_ENABLED=false` gate. The model still cannot select a connection/member, approve, or publish; the user must promote into the existing immutable `social_publish_linkedin` preview and approve separately. This initial implementation is intentionally production NO-GO pending its own live staging, activation, release-ledger, scale, and recovery qualification. Organization publishing, media, social reads, scheduling, edits/deletes, engagement actions, and direct Agent publishing remain separate future decisions.

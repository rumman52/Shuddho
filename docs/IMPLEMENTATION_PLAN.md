...
The next progression-integrity increment adds a ledger-verified post-recovery observation epoch to [controlled cohort canary progression](CONTROLLED_COHORT_CANARY_PROGRESSION.md): the historical STOP remains immutable, but only health generated after the recorded recovery verification may count toward later expansion. The recovered stage must re-earn every original window, duration and sample minimum; without fresh post-recovery health it remains HOLD. Subjective quality judging, model-selected external actions and multi-agent delegation remain later increments.

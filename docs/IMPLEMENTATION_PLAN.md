...
The following release-safety increment adds [Controlled Staging Live Approved Email Attachments](CONTROLLED_STAGING_ACTION_ATTACHMENTS.md) and [Approved Attachment Production Activation](ACTION_ATTACHMENTS_ACTIVATION.md). A live Google or Microsoft test must use an operator-selected synthetic artifact, preserve the exact v3 attachment manifest and approval-scope v2 through explicit approval, reject a wrong preview hash, execute once, and obtain provider acceptance. Production activation then SHA-binds that timestamped evidence and the reviewed rollout to the exact deployed revision, authenticates the remote runtime manifest, verifies `action_attachments=true` with controlled-cohort enforcement, and requires fresh clean post-deploy health. The flag remains off by default; release-ledger plus scale/recovery consumption of this new attestation is the next bounded enforcement increment.

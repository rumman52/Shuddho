...
The following release-evidence increment adds schema v8 `action_proposals_verified` to the same tamper-evident controlled-cohort ledger. It SHA-binds the exact live proposal proof, reviewed rollout manifest, reviewed deployment, post-deploy operator status, and activation artifact; independently re-verifies the exact normalized runtime capability/provider snapshot; rejects duplicate activation recording; and requires the chain to have already reached the declared current stage.
...
The following release-safety increment adds [Controlled Staging Live Approved Email Attachments](CONTROLLED_STAGING_ACTION_ATTACHMENTS.md) and [Approved Attachment Production Activation](ACTION_ATTACHMENTS_ACTIVATION.md). A live Google or Microsoft test must use an operator-selected synthetic artifact, preserve the exact v3 attachment manifest and approval-scope v2 through explicit approval, reject a wrong preview hash, execute once, and obtain provider acceptance. Production activation then SHA-binds that timestamped evidence and the reviewed rollout to the exact deployed revision, authenticates the remote runtime manifest, verifies `action_attachments=true` with controlled-cohort enforcement, and requires fresh clean post-deploy health.
...
The next bounded Wave D action increment adds [Approved Calendar Reminders](APPROVED_CALENDAR_REMINDERS.md): an explicit user-selected single reminder is bound into the existing immutable calendar preview and approval scope, executed through the existing Google/Microsoft calendar write permissions, and receipt-checked for exact minutes. The capability is default-off behind `SHUDDHO_ACTION_REMINDERS_ENABLED=false`; Agent proposals remain reminder-blind.

The following release-safety increment adds provider-specific live Google/Microsoft reminder evidence, production activation bound to the exact deployed revision/runtime and clean cohort health, schema-v10 `action_reminders_verified` release-ledger attestation, and mandatory consumption of that proof during bounded scale and post-rollback recovery. Reminder-enabled scale/recovery evidence upgrades to schema v5, while reminder-disabled historical paths remain backward-compatible.
...
PR #198 extends the canonical capability contract beyond final cohort admission into production activation, release-ledger attestation verification, and recovery configuration validation. Optional capability shape is normalized by one shared helper so historical rollout evidence that predates newer disabled capabilities remains verifiable while any explicit true/false drift still fails closed.
...
The next release-control enforcement increment adds [Release Activation Bundle Scale and Recovery Enforcement](RELEASE_ACTIVATION_BUNDLE_ENFORCEMENT.md). The schema-v16 writer now independently re-derives the canonical production activation set before attesting a bundle. New bounded-scale and post-rollback recovery verification require that exact bundle as additive schema-v11 evidence while preserving every feature-specific attestation. Recovery additionally requires the bundle and every activation reference it carries to be later than the exact rollback-completion event. Historical scale/recovery evidence remains verifiable under its original schema.

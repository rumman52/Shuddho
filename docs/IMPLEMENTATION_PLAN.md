...
The next activation-control increment adds [bounded scale activation verification](CONTROLLED_COHORT_SCALE_ACTIVATION.md): after a separately approved deployment change, Shuddho verifies that backend cohort enforcement remains enabled, the deployed cohort ceiling exactly matches the reviewed ceiling, membership expanded but did not exceed that ceiling, an allowed account is admitted, a fresh non-member is denied before provisioning, and post-deploy health is clean. A schema-v4 release-ledger event then binds the reviewed decision, deployment record, operator status, and activation proof into the existing tamper-evident chain. Model-selected external actions and multi-agent delegation remain later increments.
